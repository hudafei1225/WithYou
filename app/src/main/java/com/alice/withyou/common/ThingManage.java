package com.alice.withyou.common;
...
public class ThingManage {
    ...
    static {
        ...
        String[] stringArray = MyApplication.getApp().getResources().getStringArray(R.array.thing_array);
        List<String> strings = Arrays.asList(stringArray);

        for (int i = 0; i < stringArray.length; i++) {
            defaultThings.add(new Thing(i+1, stringArray[i], i+1, 0));
        }
...
//        defaultThings.add(new Thing(42, "相互诚实", 42, 0));
     ...
     * @param thing
     */
    public boolean deleteThings(Thing thing) {


        return thingDao.deleteCache("name = ?", new String[]{thing.name});
    }
}
